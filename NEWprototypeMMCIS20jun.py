...
#FUNC: defines the indexes for reading the dishes of each category
...
def start(update: Update, context: CallbackContext) -> int:
    ...
    ## user evaluation answer as a global variable
...
def userChoiceModality(update: Update, context: CallbackContext) -> int:
    ...
    print("User ", str(update.message.from_user.id), " choise ", usersGlobalVariables[str(update.message.from_user.id)]["userChoiceMacroCategoryGlobal"], " category\n")
...
def processing(update: Update, context: CallbackContext) -> int:
    ...
    tfIdfIngrNames, tfIdfDishesNames, tfIdfMenu,attractiveNames = returnFilesNamesByMacroCateg(usersGlobalVariables[str(update.callback_query.from_user.id)]["userChoiceMacroCategoryGlobal"])
    ...
    # print("the user profile----->",userProfile)
    ...
    # print('user profile updated------->', updatedDf2)
    ...
    recommendation = []
    # print("List of recommedations----->", recommendation)
    ...
    print("List of recommedations----->", recommendation[:2])
    ...
    print('recommendationObjectList', usersGlobalVariables[str(update.callback_query.from_user.id)]["recommendationObjectList"][0].nome)
    ...
    if usersGlobalVariables[str(update.callback_query.from_user.id)]["flagTextualVisualChoice"] == True:
        ...
        if usersGlobalVariables[str(update.callback_query.from_user.id)]["userChoiceModalityGlobal"] == 'Rate dish proposal (visual+label explain) ✅':
            ...
            # update.effective_message.reply_media_group(media=[InputMediaPhoto(open(image,'rb')),InputMediaPhoto(open("./" + str(usersGlobalVariables[str(update.callback_query.from_user.id)]["dishToRecommend"].FSAlabel), 'rb'))])
            ...
            # print("--- most Similar and healthy", mostSimAndHeal.nome)
            # print("--- most Similar and healthy", mostSimAndHeal.immagine)
            
            # update.effective_message.reply_text(raccHealthierNome, reply_markup=ReplyKeyboardRemove())
            # update.effective_message.reply_media_group([InputMediaPhoto(mostSiAndHeal.immagine),InputMediaPhoto(open("./" + str(mostSimAndHeal.FSAlabel), 'rb'))])
            ...
            print('Im here in this conditin right now ---- visual ----->')
            ...
            # image = "./attractive_snacks/Best Ever Jalapeno Poppers.jpg"
            # update.effective_message.reply_media_group([InputMediaPhoto(open(image,'rb')),InputMediaPhoto(mostSimAndHeal.immagine)])
    ...
    #     userResponse = update.message.text
        ...
        if usersGlobalVariables[str(update.message.from_user.id)]["nextInd"] < len(usersGlobalVariables[str(update.message.from_user.id)]["recommendationObjectList"])-1:
            ...
            if usersGlobalVariables[str(update.message.from_user.id)]["flagTextualVisualChoice"] == True:
                ...
                if usersGlobalVariables[str(update.message.from_user.id)]["userChoiceModalityGlobal"] == 'Rate dish proposal (visual+label explain) ✅':
                    # SCEN 4 multi-modal (dish name + image + label for expl) or MMLAB
                    update.message.reply_media_group(media=[InputMediaPhoto(usersGlobalVariables[str(update.message.from_user.id)]["dishToRecommend"].immagine),InputMediaPhoto(open("./" + str(usersGlobalVariables[str(update.message.from_user.id)]["dishToRecommend"].FSAlabel), 'rb'))])
                    raccHealthierNome = 'But I also propose to you a healthier alternative: ' + mostSimAndHeal.nome  # BUT I PROPOSE YOU ALSO AN HEARTIER ALTERNATIVE
                    update.message.reply_text(raccHealthierNome, reply_markup=ReplyKeyboardRemove())
                    update.message.reply_media_group([InputMediaPhoto(mostSimAndHeal.immagine), InputMediaPhoto(open("./" + str(mostSimAndHeal.FSAlabel), 'rb'))])
                else:
                    # SCEN 3 - multi-modal (dish name + image) or MM
                    raccHealthierNome = 'But I also propose to you a healthier alternative: ' + mostSimAndHeal.nome  # BUT I PROPOSE YOU ALSO AN HEARTIER ALTERNATIVE
                    update.message.reply_text(raccHealthierNome, reply_markup=ReplyKeyboardRemove())
...
def choicesatisfaction_2(update: Update, context:CallbackContext) -> int:
    ...
    return CHOICESATISFACTION3

def choicesatisfaction_3(update: Update, context:CallbackContext) -> int:
...
def choice_difficuly(update: Update, context: CallbackContext) -> int:
    global usersGlobalVariables
    userResponse = update.message.text
...
def choice_difficuly1(update: Update, context: CallbackContext) -> int:
    global usersGlobalVariables
    userResponse = update.message.text
...
def perceived_effort(update: Update, context: CallbackContext) -> int:
    global usersGlobalVariables
    userResponse = update.message.text
...
def perceived_effort1(update: Update, context: CallbackContext) -> int:
    global usersGlobalVariables
    userResponse = update.message.text
...
# def choicedifficulty(update: Update, context:CallbackContext) -> int:
    
    
#     return perceived_effort


# def perceivedeffort(update: Update, context:CallbackContext) -> int:
...
def finalRatings(update: Update, context: CallbackContext) -> int:
    ...
    if nLikes == 5:
        ...
        userEvaluAnswer = [
            ...
            usersGlobalVariables[str(update.message.from_user.id)]["PerceivedEffort"][0],
            usersGlobalVariables[str(update.message.from_user.id)]["PerceivedEffort"][1],
            usersGlobalVariables[str(update.message.from_user.id)]["PerceivedEffort"][2],
        ]
    ...
    f = open('user_eval.csv', "a")
    writer = csv.writer(f, delimiter=',')
    writer.writerow(userEvaluAnswer)
    f.close()
...
def main() -> None:
    ...
    updater = Updater("5736158886:AAG47_9V9ttJG99Hjppq5hX4qDOAyxFH_nk")
    #Updater("2116832004:AAEtkldnVaIEQ0IrZOOROcnstCU4Yo_hHpA")
    ...
    # A handler to hold a conversation with a singl or multiple users through Telegrame updates
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler('start', start)],
        states={
            USERCHOICEMODALITY: [MessageHandler(Filters.text, userChoiceModality)],  # MessageHandler: handle Tele messages. They maing contain text, media, or status updates
            ...
            CHOICEDIFFICULTY: [MessageHandler(Filters.text, choice_difficuly)],
    ...
    # Start the Bot
    updater.start_polling(timeout=600) #timeout=600
